...
import time
import random
from models.ModResNet50 import BoxResNet502
from models.HalfBoxResNet import boxresnet50
from torch.utils.data.sampler import SubsetRandomSampler

def main():
    print('File alterred 5/27/19 6:58 PM')
    ...
    test_set = torchvision.datasets.ImageFolder(root = '../Code/data/ImageNet/tiny-imagenet-200/val/images', transform=
                                                 transforms.Compose([transforms.ToTensor(),
                                                        transforms.Normalize(mean =[.4802486, .44807222, .39754647],
                                                                             std = [.2769859, .26906505, .2820814])]))
    np.random.seed(1)
    ...
    val_loader = torch.utils.data.DataLoader(train_set, batch_size = 64, 
    ...
    res_net = boxresnet50()
    res_net2 = BoxResNet502()
    
    
    
    x = train_set[0][0].unsqueeze_(0)
    res_net(x)
    
    #res_net = models.resnet18()
    res_net.to(device)
    loss_fcn = torch.nn.CrossEntropyLoss()
    optimizer = torch.optim.SGD(res_net.parameters(), lr = 0.001, weight_decay = 1e-3, momentum = .9)
    #optimizer = torch.optim.Adam(res_net.parameters(), lr = .00001, weight_decay = 0.0001)
