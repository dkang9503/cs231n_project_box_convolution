...
def main():
    ...
    folder_dir = './../../Results/BoxPSP_' + 'lr' + str(lr) + '_wd' + str(weight_decay) \
    ...
    #CREATING DIRECTORY TO SAVE ALL FILES
    if not tuning and save_idx:    
        ...
        if not resume_train:
            ...
            model = BoxResnet50_8s_psp(num_classes = n_classes)
            model.to(device)
            ...
            #scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size = lr_step, gamma = lr_step_gamma )
            e = 0
            
        else:    
            print("Directory " , folder_dir ,  " already exists")   
            model = BoxResnet50_8s_psp(num_classes = n_classes)
            model.to(device)
            loss_fcn = torch.nn.CrossEntropyLoss(ignore_index = 255)
            loss_fcn.to(device)    
    ...
    if not resume_train:
        ...
        with open(folder_dir +'/BoxPSPtrain_loss.pkl', 'wb') as handle:
            train_loss = pickle.load(handle)
        with open(folder_dir +'/BoxPSPvalid_loss.pkl', 'wb') as handle:
            valid_loss = pickle.load(handle)
        with open(folder_dir +'/BoxPSPtrain_class_iou.pkl', 'wb') as handle:
            train_class_iou = pickle.load(handle)
        with open(folder_dir +'/BoxPSPtrain_total_iou.pkl', 'wb') as handle:
            train_total_iou = pickle.load(handle)
        with open(folder_dir +'/BoxPSPtrain_total_acc.pkl', 'wb') as handle:
            train_total_acc = pickle.load(handle)
        with open(folder_dir +'/BoxPSPvalid_class_iou.pkl', 'wb') as handle:
            valid_class_iou = pickle.load(handle)
        with open(folder_dir +'/BoxPSPvalid_total_iou.pkl', 'wb') as handle:
            valid_total_iou = pickle.load(handle)
        with open(folder_dir +'/BoxPSPvalid_total_acc.pkl', 'wb') as handle:
            valid_total_acc = pickle.load(handle)
        with open(folder_dir +'/BoxPSPepoch_time.pkl', 'wb') as handle:
    ...
    while e < epochs:        
        ...
        if not tuning and save_idx:
            if e % 20 == 0 and e > 0:
                ...
                torch.save(state, folder_dir + '/modelstateHB.pth')
