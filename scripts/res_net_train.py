...
def main():
    ...
    train_loss = []
    valid_loss = []
    train_acc1 = torch.tensor([])
    train_acc5 = torch.tensor([])
    valid_acc1 = torch.tensor([])
    valid_acc5 = torch.tensor([])
    ...
    for e in range(epochs):
        ...
        for t, (x,y) in enumerate(train_loader):            
            ...
            torch.cat((iter_train_acc1,acc1))
            torch.cat((iter_train_acc5,acc5))
        ...
        torch.cat((train_acc1, torch.mean(iter_train_acc1)))
        torch.cat((train_acc5, torch.mean(iter_train_acc5)))
        ...
        with torch.no_grad():
            ...
            for t, (x, y) in enumerate(val_loader):               
                ...
                torch.cat(iter_valid_acc1.cat, acc1)
                torch.cat(iter_valid_acc5.cat, acc5)
                    
                
            valid_loss.append(np.mean(iter_valid_loss))
            torch.cat((valid_acc1,torch.mean(iter_valid_acc1)))
            torch.cat((valid_acc5,torch.mean(iter_valid_acc5)))
